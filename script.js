...
startButton.addEventListener("click", () => {
    ...
    if (appleCount > 0 && appleCount <= 20) {
        ...
        // No es necesario gameContainer.classList.remove("hidden"); si ya está visible

        // Reinicio completo al iniciar desde el menú
...
// 🆕 Event Listeners de Game Over
retryButton.addEventListener("click", () => {
    restartGame(true); // Reintentar: mantiene el número de manzanas
});

menuButton.addEventListener("click", () => {
    restartGame(false); // Menú Principal: vuelve al menú de configuración
});
...
// 🔁 REINICIAR EL JUEGO (Nueva Función)
function restartGame(shouldRetry) {
    ...
    snake = [{ x: tileSize * 10, y: tileSize * 10 }];
    direction = { x: tileSize, y: 0 }; // Dirección inicial para que la serpiente se mueva inmediatamente
    newDirection = { x: tileSize, y: 0 };
    foods = [];
    obstacles = [];
    score = 0;
    ...
    if (shouldRetry) {
        // Opción "Reintentar": Inicia directamente con la misma configuración de manzanas
        initializeGame();
    } else {
        // Opción "Menú Principal": Muestra el menú de inicio para cambiar la configuración
...
function endGame() {
    ...
    }, 500); // Pequeño retraso para reproducir el sonido de 'lose'
}
